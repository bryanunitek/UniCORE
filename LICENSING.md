---
title: "UniCORE Licensing Reference — Formal Statement and Worked Scenarios"
author: Bryan Fred, Unitek Systems Limited, Bedford, United Kingdom
version: "DRAFT v0.01 · May 2026"
status: DRAFT v0.01
licence: CC BY 4.0
---

# UniCORE Licensing Reference — Formal Statement and Worked Scenarios

**Author:** Bryan Fred, Unitek Systems Limited, Bedford, United Kingdom
**First published:** May 2026
**Status:** DRAFT v0.01 — published for review and comment via [GitHub Discussions](https://github.com/bryanunitek/UniCORE/discussions)
...
This document is a formal reference you can give to a **Partner, Client, or Software Provider** who asks how the UniCORE licensing model works.
...
- Enterprise software companies serving named industries (Law, Banking, Healthcare, Space Industry)
- Small, Medium, and Large software companies that want **fewer than the standard 12 Levels of Governance** (floor of 4)
...
- Each named industry: Law, Banking, Healthcare, Space Industry

Where this document and the canonical license text disagree, the canonical license text wins. This document is plain-English guidance, not a substitute for the LICENSE file in each repository.
...
| Redistribute it | Indicate what you changed | Add legal or technical restrictions that prevent others from doing the same |
...
1. **Attribute the author.** The canonical attribution line is:
   ...
   > *"Based on UniCORE, by Bryan Fred, Unitek Systems Limited (CC BY 4.0). https://github.com/bryanunitek/UniCORE"*

   The attribution applies per-repository. If you also use UniCORE.GVB or one of the Vertical COREs, add their attribution lines too.
...
4. **Not imply endorsement.** Do not say or imply that Bryan Fred, Unitek Systems Limited, or any author of UniCORE endorses, supports, or is responsible for your derivative.
...
The remaining sections walk through each scenario you are likely to be asked about. Each scenario follows the same structure:

1. **Profile** — who they are
2. **What they take from the gift layer**
3. **What they build on top**
4. **What the licence requires of them**
5. **What they cannot do**
6. **Worked example with specifics**

---

### Scenario 5.1 — Enterprise software company in a named industry

**Profile:** A multi-product enterprise software vendor — examples: a major legal-tech vendor (Law); a core-banking platform vendor (Banking); an EHR / EMR vendor (Healthcare); a mission-systems vendor (Space Industry). Revenue measured in tens or hundreds of millions. Long sales cycles. Regulated customers.
...
- The matching **Vertical CORE** (UniCORE.Law / UniCORE.Banking / UniCORE.Healthcare / UniCORE.Space-Industry) once certified
...
- Keep their fork of any UniCORE-licensed code under CC BY 4.0 if they redistribute it. If they only use it internally without redistribution, attribution is still required on any product that ships including those parts.
...
**Worked example — Acme LegalTech Plc:**

Acme is a £200M legal-tech vendor. Their flagship product is a matter-management platform with a billing module. They take:

- UniCORE-AI for the 12-Level Governance pattern
- UniCORE.Law (after certification) as the Vertical CORE reference

They re-architect their billing module so its workflow engine, audit trail, and approval gates conform to the UniCORE pattern. They keep their own conflict-search engine (their commercial differentiator) but plug it into the Vertical CORE conflict-clearance contracts.

They ship Acme Legal Platform v8 with:

- A NOTICE.md attributing UniCORE-AI and UniCORE.Law
- A LICENSE-3RD-PARTY.md including the CC BY 4.0 text for the UniCORE-derived portions
- A "Built on the UniCORE.Law pattern" line in their product documentation

They charge their clients £400k per seat per year. None of that revenue is owed to Unitek, because the gift layer is — by design — free for commercial reuse.

If Acme later wants the "**Powered by UniCORE AI**" badge or the "**UniCORE.Law-certified**" badge, they go through the published certification process. Until then, they describe themselves as **building on** the pattern, not as **certified to** it.

---

### Scenario 5.2 — Small, Medium, and Large software companies using fewer than 12 Levels (floor = 4)

**Profile:** A software vendor that does not need the full 12-Level Governance stack — common when the customer base is small-business or mid-market, or the regulatory load is light, or the product is point-functional (e.g., a single-purpose SaaS tool).
...
- **Levels skipped:** Multi-jurisdiction routing, Board-level governance, regulator-facing escalation, etc. — out of scope for the product.
...
- Their own Vertical Business Objects in private working repositories (`UniCORE.Law-Claw`, `UniCORE.GVB-Claw`, etc. — Unitek's commercial differentiator)
...
1. **Decades of experience** in the vertical — 33 years of legal IT in Bryan's case (since February 1993)
...
Unitek USA hosts the production-grade UniCORE.GVB deployments at the operator tier. Customers contract with Unitek USA for the hosting service. The hosting agreement:

- Names the public CORE components in play and links to their LICENSE files
- Names Unitek's private Vertical Business Object packages and their commercial licence terms
- Separates the **service fee** (hosting + operations + SLA) from any **code licence** (none for CORE, separate commercial agreement for the Vertical Business Objects)

A competitor — including Acme LegalTech from Scenario 5.1 — could in principle stand up the same public CORE and host the same kind of Solution. Unitek's competitive position is on operational quality, hardware ownership, and the 30-year-plus relationship with the underlying business stack — **not** on exclusive access to the code.
...
**Licence implications:** The law firm doesn't have to know or care about the licence terms in detail — those obligations sit with the service company. The service company attributes; the firm uses.

**What the firm cannot do (in either route):**

- Re-sell UniCORE.Law as their own product without attribution.
- Add a clickwrap that purports to grant their employees fewer rights to the underlying CC BY 4.0 code than the licence itself grants (which is fine in practice — employees don't usually need the underlying rights — but worth noting).
...
### Scenario 5.6 — Organisations requiring 13+ Levels of Governance

**Profile:** Heavily regulated, multi-jurisdiction, or life-safety deployments where the 12-level reference is insufficient. Examples: a global investment bank with parallel regulator regimes; a multi-national hospital network with country-specific medical-device approval flows; a space-industry contractor with ITAR / EAR / export-control overlay; a defence supplier (in any non-Military-named programme).
...
- Additional governance levels — typically jurisdiction-specific, regulator-specific, or domain-specific. Example: Levels 13–15 for a space-industry contractor might be ITAR review, NASA Class-A flight-readiness sign-off, and post-flight forensic review.
...
- If they merge their additional levels back into the public UniCORE-AI repository as a contribution, the merged portion must be CC BY 4.0 (that is the licence of the receiving repository).
...
> *"This deployment implements UniCORE AI 12-Level Governance plus three additional levels (ITAR review, flight-readiness, post-flight forensic) added by Acme Aerospace Plc. The UniCORE AI 12-Level portion is used under CC BY 4.0; the additional three levels are © Acme Aerospace Plc, all rights reserved."*

**What they cannot do:**

- Renumber or rename the original 12 levels in a way that creates confusion about what the public reference says.
- Claim the additional levels are part of UniCORE AI as published — they are derivative additions and must be marked as such.
- Use a reserved naming prefix for the extension (e.g., calling their extended framework "UniCORE.Military" — Military is reserved out of the gift surface entirely, and that reservation is enforced at the public corpus level via STATEMENT-ON-CLAIMS).

**Worked example — Globex Aerospace:**

Globex builds avionics control software for crewed spacecraft. They adopt UniCORE AI's 12-Level Governance and add three further levels:

- Level 13 — ITAR/EAR export classification review
- Level 14 — NASA / ESA flight-readiness review board
- Level 15 — Post-flight forensic and lessons-learned

Globex publishes a paper at an industry conference describing their 15-level adaptation. The paper attributes UniCORE AI for levels 1–12 and credits Globex for 13–15. They release the level-13–15 framework under their own licence (could be proprietary, could be Apache, could be CC BY 4.0 — their call).

A regulator audit finds the deployment compliant. Globex says "Powered by UniCORE AI" only if they are formally certified; otherwise they say "**built on the UniCORE AI 12-Level reference, with three additional levels for our domain**". Either statement is honest under the licence.

---

### Scenario 5.7 — Organisations using fewer than 4 Levels of Governance (<4)
...
**What the licence permits:** Anything CC BY 4.0 permits — i.e., they can take whichever levels they want, in whatever combination they want, and use them.

**What the public corpus advises against:**

- A production deployment with fewer than 4 levels is not, in our view, a deployment that has earned the right to claim conformance with the UniCORE AI framework.
- A regulator-facing deployment with fewer than 4 levels is, in our view, not a serious governance posture and should not be described as "Powered by UniCORE AI" or "TrueAI-certified".
...
- A deployment with <4 levels will not pass certification.

**Worked example — University of Bedford CS-301 coursework:**
...
If, three years later, one of those students tries to commercialise their coursework as a real product with only those two levels, they would be welcome to do so — and the licence permits it — but they would not pass certification, would not be entitled to use the certification marks, and would (in our published view) not be running a production-grade governance posture. The public corpus will say so if asked.

---

## 6. Worked scenarios by named industry

The four industries below are the named verticals on the public roadmap as of 2026-05-22. Other industries (Government, Accountancy, etc.) follow the same pattern.
...
- Time-and-billing is the operational engine — Bryan's 33-year deep specialism (since February 1993).
...
- The Vertical CORE pattern is the gift; the Vertical Business Objects (Law-specific time-recording rules, jurisdiction-specific billing formats, firm-specific approval matrices) are typically commercial — kept in private working repositories like `UniCORE.Law-Claw`.

**Who uses it:**

- Legal-tech vendors building competing products
- Law firms running it themselves
- Service companies (like Unitek) configuring and operating it for law-firm clients
...
- The naming prefix `UniCORE.Law.Military` is reserved out — Military is excluded from the public corpus entirely, and so are its industry sub-prefixes.

**Worked example — Northwind & Partners LLP (illustrative UK + US law firm):**

Northwind & Partners is an illustrative law firm chosen for this worked example only — not a real client. If a firm of this profile engaged Unitek as the service company, the deal would look like:

1. The firm signs a services agreement with Unitek Systems Limited (UK) and/or Unitek Systems USA Inc (US).
2. Unitek deploys UniCORE.Law (gift, CC BY 4.0) plus its private `UniCORE.Law-Claw` Vertical Business Object package (commercial, separately licensed).
3. The deployment runs on Unitek USA's GVB Production tier (hosting service, separately billed).
4. The firm pays: (a) the hosting service fee, (b) the Vertical Business Object licence fee, (c) the implementation and ongoing professional services fee. They pay nothing for the public UniCORE.Law portion — that is gift.
5. The firm's deliverable documentation pack includes the LICENSE files for all the public CORE components in play.
...
- Multi-jurisdiction regulator overlay (FCA, PRA, OCC, FDIC, ECB, FINMA, MAS, HKMA, etc., depending on footprint)
...
- A core-banking vendor implementing UniCORE.Banking will typically need 13+ levels (see Scenario 5.6) because of the multi-jurisdiction regulator overlay — and the licence permits that.
- The vendor's own implementations of regulator-specific filings are their own work, can be proprietary or otherwise licensed at their discretion.

**Who uses it:**

- Core-banking platform vendors (Temenos-class, Mambu-class)
...
A 5-year-old neo-bank with banking licences in three EU countries runs its compliance and audit stack on UniCORE.Banking. They implement 14 levels — the standard 12 plus two for SEPA-specific settlement-finality forensics and one for cross-border tax-information-exchange (CRS / FATCA).

Their attribution line in their compliance documentation:

> *"Compliance stack based on UniCORE.Banking by Bryan Fred, Unitek Systems Limited (CC BY 4.0). Two additional governance levels added for SEPA settlement-finality forensics and cross-border tax-information-exchange. © Neo-Bank Plc, all rights reserved on the additions."*

The bank pays nothing for the public UniCORE.Banking portion. They invest engineering effort to integrate, configure, and maintain it.

If the bank's compliance tooling becomes good enough that they want to spin it out as a sellable RegTech product, they can do so under CC BY 4.0 — same terms, same attribution requirements, same freedom to commercialise.
...
If a vendor — say, a pharma company — later wants to use the same hospital network's adapted level-13 framework, the hospital can publish that level-13 framework under whatever licence they choose. They might even publish it under CC BY 4.0 as a contribution to the global healthcare community. Their choice; the licence does not force them either way.
...
- Multi-agency coordination (NASA / ESA / JAXA / CSA / ISRO / CNSA, plus commercial operators like SpaceX, Blue Origin, Rocket Lab, etc.)

**The Positioning Principle:**

The public CORE programme — UniVERSE / TrueAI / UniCORE-AI / UniCORE / Vertical COREs — is positioned for **Harmony, Peace, Space Exploration, and Humanity**.

`UniCORE.Military` and any military-named industry prefix is **deliberately reserved out** of the public corpus. Space Industry as named here means civil and commercial space, not weapons systems.

**Licensing implications when you take UniCORE.Space-Industry:**

- Same CC BY 4.0 terms.
- Space-industry deployments commonly need 13+ levels (see Scenario 5.6 — Globex Aerospace example).
- The Positioning Principle is enforced at the public corpus level, not at the licence level. CC BY 4.0 itself does not restrict use to non-military purposes — but the public statements, the named-industry list, and the reserved-prefix list make Unitek's position clear: we publish these for civil and commercial space, and we will not certify, badge, or endorse military-classified derivatives.
...
A commercial constellation operator (say, ~80 satellites in low Earth orbit, 24/7 imagery service to civilian customers) adopts UniCORE.Space-Industry for its mission-control governance and customer-data audit framework. They implement 14 levels — 12 reference + 1 for ITAR / EAR export-control on imagery distribution + 1 for orbital-debris-mitigation reporting under the long-term sustainability guidelines.

They publish a NOTICE.md attributing UniCORE.Space-Industry, UniCORE-AI, and TrueAI Foundation. Their levels 13–14 are kept internal, proprietary, because they are commercially sensitive (export-control classifications and orbital-debris models reveal customer identity and competitive positioning).

They pay nothing for the public CORE portion. They sell their imagery service commercially. The licence permits this.

If the operator later wins a defence contract, the **defence work** is not within the scope of the public UniCORE.Space-Industry positioning. They can fork privately, the licence permits the fork — but they cannot describe the defence-classified deployment as "Powered by UniCORE AI" if the public corpus's reserved-marketing-phrase rules disqualify defence applications. (As of 2026-05-22 the corpus's position is: civil and commercial space yes; weapons systems no.)
...
- **"CC BY 4.0 means it's only for non-commercial use."** False. CC BY 4.0 explicitly permits commercial use. The "BY" stands for "Attribution", not "non-commercial". (CC BY-NC would be non-commercial; we are not using it.)
...
- **"The licence covers the trademarks too."** False. CC BY 4.0 covers copyright, not trademarks. The "Powered by UniCORE AI" mark, the "TrueAI-certified" mark, and so on are governed separately by the published statement-on-claims and certification policy, not by the CC BY 4.0 licence text.
...
- **"Unitek can revoke the licence later."** False. CC BY 4.0 is irrevocable. Once a version is published under CC BY 4.0, that version is forever available under those terms. Future versions could in principle be released under different terms — but that does not retroactively un-license what is already out there.

---

## 8. Things the public corpus enforces beyond the licence

CC BY 4.0 itself does not cover everything. Some matters are enforced by the public corpus through statements, conventions, and refusal to certify or endorse — not through the licence text:

| Matter | Enforced by | Mechanism |
|---|---|---|
| Reserved marketing phrases (`Powered by UniCORE AI`, `TrueAI-certified`, etc.) | STATEMENT-ON-CLAIMS in each public repository | Public refutation if used incorrectly; certification policy |
| Reserved naming prefixes (`UniCORE.Military`, etc.) | STATEMENT-ON-CLAIMS | Public refutation |
...
| Honest scope claims (don't claim 12 levels if you have 4) | Published guidance and the certification gate | Certification refusal; public clarification if asked |
...
| Field | Value |
|---|---|
| Document version | DRAFT v0.01 |
| First published | May 2026 |
| Author | Bryan Fred, Unitek Systems Limited, Bedford, United Kingdom |
| Licence of this document | Creative Commons Attribution 4.0 International (CC BY 4.0) |
| Review status | DRAFT v0.01 — corrections and clarifications welcome via [GitHub Discussions](https://github.com/bryanunitek/UniCORE/discussions) |
